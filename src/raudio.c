...
bool RaylibIsWaveValid(RaylibWave wave)
{
    bool result = false;
...
bool RaylibIsSoundValid(RaylibSound sound)
{
    bool result = false;
...
RaylibMusic RaylibLoadMusicStream(const char *fileName)
{
    ...
    else if (RaylibIsFileExtension(fileName, ".flac"))
    {
        ...
        if (ctxFlac != NULL)
        {
            ...
            if (ctxFlac->bitsPerSample == 24) sampleSize = 16;   // Forcing conversion to s16 on RaylibUpdateMusicStream()
            music.stream = RaylibLoadAudioStream(ctxFlac->sampleRate, sampleSize, ctxFlac->channels);
...
bool RaylibIsMusicValid(RaylibMusic music)
{
...
bool RaylibIsAudioStreamValid(RaylibAudioStream stream)
{
